...
public class Hud {
	...
	public void render(ShapeRenderer sr) {
		 ...
		 * if(item!=null) sr.setColor(Color.GREEN); else sr.setColor(Color.RED);
		 * //TODO square around the selected item if(true){ Vector2 pos=new
		 * Vector2(i*55+300 ,textCam.viewportHeight/2-50); sr.rect(pos.x
		 * ,pos.y,50,50); } }
		 */
	}
	...
	public void render(SpriteBatch sb) {
		 ...
		 * for(i=0;i<gameState.player.inventory.inventory.length;i++){
		 * InventoryItem item= gameState.player.inventory.inventory[i];
		 * if(item!=null) sb.draw(item.getTexture(), i*55+300
		 * ,textCam.viewportHeight/2-50,50,50); }
		 */
		/*
		 * for(i=0;i<gameState.player.inventory.inventory.length;i++){
		 * InventoryItem item= gameState.player.inventory.inventory[i];
		...
		font.draw(sb, "Mouse Position: " + format.format(mousePos.x) + "," + format.format(mousePos.y), -600,
				textCam.viewportHeight / 2 - 20);
